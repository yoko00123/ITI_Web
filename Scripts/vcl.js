			...
			(function (vclExports, amdRequire) {
			    ...
			    vcl.Path = (function () {
			        return {
			            ...
			            GetPage: function (fle) {
			                ...
			                return vcl.Data.Parse(vcl.Path.GetFile(fle), 'text/html');
			            },
			    ...
			    vcl.Array = (function () {
			        return {
			            ...
			            Contains: function (obj, o, comparer) {
			                comparer = comparer || vcl.Array.DefaultEqualityComparer;
			                var l = obj.length;
			                while (l-- > 0)
			                    if (comparer(obj[l], o) === true) return true;
			                return false;
			            },
                ...
                */
			    ...
			    vcl.Require = (function (pth, script, tag) {
			        ...
			        if (script instanceof Array) {
			            for (v in script)
			                Inject(vcl.Path.Combine(pth, script[v]), script[v], tag);
			        } else
			    ...
			    /*Nat20160530*/
			    vcl.SystemParameters = (function () {
			        vcl.Core.call(this);

			        var options = vcl.Options;
			        if (options.SystemParameters.length > 0) return;

			        var param = [];
			        var dt = vcl.DateTime.ShortDate2();
			        param.push({ Key: '@FromDate', Value: dt, Index: 0 });
			        param.push({ Key: '@ToDate', Value: dt, Index: 1 });
			        param.push({ Key: '@StartDate', Value: dt, Index: 0 });
			        param.push({ Key: '@EndDate', Value: dt, Index: 1 });
			        param.push({ Key: '@Date', Value: dt, Index: 0 });
			        param.push({ Key: '@Year', Value: new Date(dt).getFullYear(), Index: 0 });

                    options.SystemParameters = param;
